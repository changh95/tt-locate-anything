...
"""LocateAnything-3B overlay of tt-metal's ``models/tt_transformers/tt/mlp.py`` (main 8b98410e730).

Identical to the stock file except for ONE change in :meth:`MLP.forward`: in DECODE mode on a
single chip (not galaxy, no prefetcher) the ``w1`` output is moved to DRAM before the ``w3``
matmul runs, and the ``w3`` output right after it, so the gate/up product is computed in DRAM
and the stock ``to_memory_config`` brings it back to L1 for ``w2``. With the stock code both
``[32, hidden_dim]`` bf16 intermediates stay L1 width-sharded while the DRAM-sharded ``w3``
matmul allocates its weight-stream circular buffers, and on one Blackhole p150a that overflows
L1 (``program.cpp: Statically allocated circular buffers ... clash with L1 buffers``, first
decode step). This is the "decode w1/w3 spilled to DRAM so the BFP8 weight-stream circular
buffers fit L1" change the port README validated with (PCC 0.9928 vs the torch reference); it
mirrors the disabled ``TG`` small-dim branch below. Prefill, galaxy and prefetcher paths are
untouched. ``extra_code`` wins over ``source.code`` on collision, so this file replaces the
stock one inside the image.
"""
...
class MLP(LightweightModule):
    ...
    def forward(self, x: ttnn.Tensor, mode: Mode) -> ttnn.Tensor:
        ...
        # LocateAnything overlay (single-chip decode): spill w1_out to DRAM *before* the w3 matmul
        # so its L1 shard is not live while w3's weight-stream circular buffers are allocated, then
        # spill w3_out too so the mul below runs DRAM->DRAM; the stock to_memory_config after the
        # mul puts w2_in back on the w2 core grid in L1. Same recipe as the disabled TG branch.
        spill_ff1_3 = mode == Mode.DECODE and not TG and self.prefetcher is None
        if spill_ff1_3:
            w1_out_l1 = w1_out
            w1_out = ttnn.to_memory_config(w1_out_l1, ttnn.DRAM_MEMORY_CONFIG)
            ttnn.deallocate(w1_out_l1)
        w3_out = ttnn.linear(
            x,
            self.w3,
            dtype=ttnn.bfloat8_b if TG else activation_dtype or ttnn.bfloat16,
        ...
        if spill_ff1_3:
            w3_out_l1 = w3_out
            w3_out = ttnn.to_memory_config(w3_out_l1, ttnn.DRAM_MEMORY_CONFIG)
            ttnn.deallocate(w3_out_l1)
        ttnn.deallocate(x)
